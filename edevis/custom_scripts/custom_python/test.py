...
from nextcloud_links import NextcloudAPI


def create_nextcloud_share(path, expire_date = datetime.now() + timedelta(days=1)):       
    #.     https://cloud.edevis.eu/ocs/v2.php/apps/files_sharing/api/v1/shares
    url = "https://cloud.edevis.eu/ocs/v2.php/apps/files_sharing/api/v1/shares"
...
# #        result = api.create_nextcloud_share("/edevis.com Software Downloads/Drivers/Signal Generator (ESGx)/ESG3/ESG USB Driver/ESG3 Driver.exe", "2025-12-01")

#         shares = api.get_all_shares()
#         for share in shares:
#             print(f"Share ID: {share['id']}, Path: {share['path']}, Expiry: {share.get('expiration', 'N/A')}") 
#             if share.get("expiration"):
#                 expire_date = datetime.strptime(share["expiration"], "%Y-%m-%d %H:%M:%S")
                
#                 if api.is_expired (share["expiration"]):
#                     print (f"  -> This share is expired!")
#                     api.delete_share(share['id'])
#                     print (f"  -> Share {share['id']} deleted.")
